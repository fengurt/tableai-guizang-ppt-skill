# Layouts · Style C · Table AI Design System (Fork of Style B)

> 🆕 **本文件 fork 自 `references/layouts-swiss.md`(Style B / 瑞士国际主义)。**
> Style C 与 Style B 的 22 个锁定版式 (`S01`-`S22`) **完全相同** —— 只在视觉/字体/圆角上做品牌化调整。
> 所以本文与 `layouts-swiss.md` **几乎 1:1 重复**。差异标注在下文每个版式的"Style C 微调"小节。
>
> 当你在看 `Style C 微调: ...` 这段话,就说明这里与 Style B 不同。

**核心差异速查**:

| 维度 | Style B (Swiss) | Style C (Table AI) |
|------|----------------|-------------------|
| 主色 / 强调色 | 克莱因蓝 IKB `#002FA7` (4 选 1) | 日晷暗金 `#A88B52` (唯一) |
| 字体 | Inter (无衬线 / vw 流式) | Manrope (无衬线 / px 定值) |
| 字重 | 极轻 200 / 300 | 半粗 600 / 700 |
| 圆角 | 严格直角 0px | **卡片微圆角 2-4px**，图片仍直角 |
| 装饰背景 | WebGL 网格 + ASCII 字符场 | WebGL 网格 + 金色 ASCII 字符场 |
| hero dark 默认 | `.slide.accent` (IKB 满屏) | `.slide.dark` (寰宇深蓝满屏) |
| hero page accent 调色 | IKB 克莱因蓝 | 暗金 + 深蓝 |

22 个原始登记版式 · 严格模块化网格 · 每个版式说明用途、骨架、关键类名、专属动效。

> ⚠️ 这套版式与风格 A(电子杂志/电子墨水)**不通用**，也 **与 Style B 通用**（同一套 `Sxx` 版式 ID），只是颜色/字体不同。一份 deck 只能选一套。
...
## Style C 微调 · 页面级约定（统一生效，无须每页重复）

下面列出的规则对 Style C 全部 22 个版式都生效。各 layout 的示例代码继续以 Style B 版本为底，但请按下表替换类名 / 字体 / 颜色：

| 维度 | Style B 默认 | Style C 替换 |
|------|-------------|--------------|
| 字体（标题） | `var(--sans)` Inter `font-weight:200` | `var(--sans-tab)` Manrope `font-weight:700` |
| 字体（kicker / chrome） | `var(--mono)` 11px caps letter-spacing:.22em | `var(--sans-tab)` 12px 600 caps letter-spacing:.16em |
| 大标题字号不变；字重升级 | h-xl weight 200 | h-xl weight 700 |
| callout / accent line | 3px 金色 | 3px 金色（保持一致） |
| `.card-fill` 圆角 | 0 | 4px |
| `.sub-card` / `.stack-block` / `.bar-tower .body-block` | 0 | 4px |
| 图片容器 | 0 直角 | 0 直角（保留 - 图像语言不变） |
| hairline / divider | 0 直角 | 0 直角（保留） |
| 强调字 | IKB blue | Sundial Gold `#A88B52` |
| hero dark 满屏 | `.slide.accent` (IKB) | `.slide.dark` (寰宇深蓝) + ASCII 字段保持金色调色板 |
| hero 收尾右半 takeaway 第 03 条 accent | IKB 蓝 | Sundial Gold |

生成前请同步运行 `node scripts/validate-tableai-deck.mjs index.html`。
