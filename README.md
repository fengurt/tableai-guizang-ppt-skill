...
- **Style C: Table AI Design System (新增 fork)**。完全继承 Style B 的 22 个版式与动效,只品牌化:寰宇深蓝 `#0A1626` + 日晷暗金 `#A88B52` + Manrope + 2-4px 微圆角。适合 Table AI 产品报告、SaaS Dashboard、KPI 汇报。详见 `references/themes-tableai.md`。
...
**新增 · Style C · Table AI Design System (fork of Style B)**

> 🆕 Style C 是 Style B 的**品牌化 fork**:22 个版式、动效系统、网格逻辑完全继承,只替换颜色 (寰宇深蓝 `#0A1626` + 日晷暗金 `#A88B52`)、字体 (Manrope)、圆角 (2-4px 微圆角)。适合 Table AI 产品报告、SaaS Dashboard、KPI 汇报。详见 `references/themes-tableai.md`。
...
- 🧱 **Style B 22 种锁定版式** / **Style C 继承同 22 个版式**:Cover、Statement、KPI Tower、Loop Diagram、Duo Compare、Image Hero、Closing Manifesto 等
- 🎨 **主题色预设**:Style A 5 套电子墨水主题,Style B 4 套瑞士高饱和锚点色,**Style C 1 套 Table AI 暗金**
...
- **Style C fork**:完全继承 22 个版式 `S01-S22`,只替换颜色 (寰宇深蓝 + 日晷暗金)、字体 (Manrope)、圆角 (2-4px 微圆角)。详见 `references/themes-tableai.md` 与 `references/swiss-tableai-layout-lock.md`。
...
两个校验器**共用** `allowedLayouts` 集合。Style C 版本额外检查 Manrope 字重、box-shadow 禁用、hex 色板白名单。
...
│   ├── template-tableai.html ← Style C Table AI Design System 模板 (新增 fork)
│   └── screenshot-backgrounds/ ← 截图美化内置背景:style-a 5 套 / style-b 4 套 / style-c (按需)
├── scripts/
│   ├── validate-swiss-deck.mjs ← 瑞士风版式校验器
│   └── validate-tableai-deck.mjs ← Style C Table AI 校验器 (新增 fork)
└── references/
    ├── components.md     ← 组件手册(字体、色、网格、图标、callout、stat、pipeline)
    ├── layouts.md        ← 10 种页面布局骨架(可直接粘贴)
    ├── layouts-swiss.md  ← 22 种瑞士风锁定版式
    ├── layouts-tableai.md ← 22 种 Style C 版式 (fork of layouts-swiss.md)
    ├── swiss-layout-lock.md ← 瑞士风还原度和版式硬约束
    ├── swiss-tableai-layout-lock.md ← Style C 视觉/排版约束 (fork)
    ├── themes.md         ← 5 套 Style A 主题色预设
    ├── themes-swiss.md   ← 4 套 Style B 锚点色
    ├── themes-tableai.md ← 1 套 Style C Table AI 暗金 + 寰宇深蓝 (fork)
...
### Style C · Table AI Design System (新增 fork)

> 🆕 Style C 是 Style B 的品牌化 fork —— 22 个版式 `S01-S22`、动效系统、网格逻辑**完全继承**自 Style B，只重写颜色 / 字体 / 圆角。详见 `references/themes-tableai.md`。

| 角色 | Token | 值 | 说明 |
|------|-------|----|------|
| 底色 | `--paper` | `#FFFFFF` | 斯堪的纳维亚纯白 |
| 主色 | `--ink` | `#0A1626` | **寰宇深蓝** (Universe Deep Blue) |
| 点缀色 | `--accent` | `#A88B52` | **日晷暗金** (Sundial Dark Gold) |
| 微光 | `--gold-mist` | `rgba(168,139,82,0.08)` | hover/active 极细金色 |
| 字体 | -- | Manrope + Noto Sans SC + JetBrains Mono | Manrope SemiBold 700 / 600 |

**核心差异**:

| 维度 | Style B | Style C |
|------|---------|---------|
| 主色 | IKB 克莱因蓝 `#002FA7` | 寰宇深蓝 `#0A1626` |
| Accent | 4 选 1(蓝/黄/绿/橙) | **唯一**: 日晷暗金 `#A88B52` |
| 字体 | Inter (vw 流式) | **Manrope** (px 定值 / 16-24px) |
| 字重 | 极轻 200 / 300 | 半粗 600 / 700 |
| 圆角 | 严格直角 | 卡片 2-4px 微圆角,图片仍直角 |
| Hero 满屏 | `.slide.accent` (IKB) | `.slide.dark` (深蓝) + 金色 ASCII 字段 |
| 强调手段 | IKB 蓝 | 暗金 + 反白 |

**适用场景**:

- Table AI 产品内部汇报 / Sprint Review
- SaaS Dashboard 截图 + 数据讲解
- KPI / OKR 季度汇报
- AI / 自动化 / B2B 产品 demo day
...
`validate-tableai-deck.mjs` 与 `validate-swiss-deck.mjs` **共用** `allowedLayouts` (S01-S22)，但额外检查：
- Style C 不得在主标题用 `font-weight: 200/300`（应保持 600/700）
- Style C 卡片不得使用 `box-shadow`
- Style C hex 不得超出 ink / gold / paper / grey ramp 白名单
...
- Style C 改动请同步更新 `template-tableai.html` + `layouts-tableai.md` + `swiss-tableai-layout-lock.md` + `scripts/validate-tableai-deck.mjs`
- Style C 是 Style B 的 fork,不应新增任何 `S23/S24` 版式 ID
- 把踩过的坑写到 `checklist.md` 对应的 P0 / P1 / P2 / P3 级别
- 新主题色进 `themes.md` / `themes-swiss.md` / `themes-tableai.md` 并给出适合的场景
