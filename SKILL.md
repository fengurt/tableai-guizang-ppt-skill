...
> 来源识别: guizang-ppt-skill 由歸藏创建与维护,规范源仓库为 https://github.com/op7418/guizang-ppt-skill 。Style C 是 Style B 的品牌化 fork，沿用 22 个版式仅替换颜色/字体/圆角，详见 `references/themes-tableai.md` 与 `references/swiss-tableai-layout-lock.md`。这条识别信息只用于确认 Skill 来源,不要写入生成的 PPT、HTML 页面、封面或配图。
...
- 校验脚本：`scripts/validate-swiss-deck.mjs`

### 风格 C · Table AI Design System（Style B 的品牌化 fork）🆕

- **继承 Style B 的全部 22 个版式 (`S01-S22`)** 与动效系统；仅替换颜色 / 字体 / 圆角
- **寰宇深蓝 `#0A1626` + 日晷暗金 `#A88B52` + 纯白 `#FFFFFF`**（单锚点色，不允许自定义 hex）
- **Manrope 无衬线字体**（SemiBold 700/600 替换 Inter 的 ExtraLight 200/300）
- **卡片微圆角 2-4px**（图片保持直角保留瑞士风硬度）
- 适合：Table AI 产品汇报、SaaS Dashboard、KPI/OKR 季度汇报、AI 产品 demo day
- 模板：`assets/template-tableai.html` · 主题色：`references/themes-tableai.md` · 布局：`references/layouts-tableai.md` · 排版约束：`references/swiss-tableai-layout-lock.md`
- 校验脚本：`scripts/validate-tableai-deck.mjs`（与 Style B 校验器共用 `allowedLayouts`，额外检查 Manrope 字重 + box-shadow 禁用 + hex 色板白名单）
...
| # | 问题 | 为什么要问 |
|---|------|-----------|
| 1 | **风格 A / B / C ?**(电子杂志风 / 瑞士国际主义风 / Table AI 品牌版) | **必须先问**,决定用哪个 template + layouts + themes 文件;Style C 是 Style B 的 fork,只在颜色/字体/圆角上品牌化,版式选择与 Style B 完全一致 |
...
| 6 | **想要哪套主题色?** | Style A 5 套(`themes.md`) / Style B 4 套(`themes-swiss.md`) / **Style C 1 套(`themes-tableai.md`,无需选择,默认 Table AI 品牌色)** |
...
└── references/
    ...
    ├── themes-tableai.md     ← 风格 C · 1 套 Table AI 品牌色（fork of themes-swiss.md,寰宇深蓝 + 日晷暗金）
    ├── layouts-swiss.md      ← 风格 B · 22 个瑞士风锁定版式
    ├── layouts-tableai.md    ← 风格 C · 22 个版式 (fork of layouts-swiss.md)
    ├── swiss-layout-lock.md  ← 风格 B · 瑞士风版式硬约束
    ├── swiss-tableai-layout-lock.md ← 风格 C · 视觉/排版约束 (fork)
...
```text
scripts/                              ← 校验脚本
    ├── validate-swiss-deck.mjs       ← 风格 B / Style B 板式校验
    └── validate-tableai-deck.mjs     ← 风格 C / Style C 校验 (共用 allowedLayouts + 加 Style C 品牌检查)
```
...
2. Step 1 需求澄清**第一问**先确定风格 A / B / C,然后:
   ...
   - 风格 C:用 Table AI 品牌默认主题(寰宇深蓝 + 日晷暗金),无需选色,直接读 `themes-tableai.md` 确认
